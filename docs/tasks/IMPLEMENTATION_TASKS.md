...
- [ ] **5.7 parse-message-tags の本番 URL 直書きと未認証の是正**（2026-09-22 発見・実装、ブランチ `fix/parse-message-tags-webhook`）
  - 発見1（ローカルが本番を呼ぶ）: トリガー関数 `call_parse_message_tags()`（最新定義 20260719000000、SECURITY INVOKER）が本番の Edge Function URL を直書きして `net.http_post` していたため、ローカル・隔離スタックでコミットされたメッセージ（Seed.sql・QA・API テスト）がすべて本番の parse-message-tags に送られていた。2026-09-22 に本番の `notification_logs` へ 18 行（kind='message'・status='skipped'・user_id = Seed.sql のプレースホルダー `11111111-…` / `22222222-…`、10:34〜10:37 UTC）が書き込まれていた。push と記録が作られなかったのは、プレースホルダー UUID が本番に存在しなかったからにすぎない
  ...
  - [ ] **オーナー作業（順序厳守）**: ① Vault の `project_url` / `secret_key` の存在確認 → ② `supabase db push`（migration）→ ③ `supabase functions deploy parse-message-tags --no-verify-jwt` → ④ 認証なし POST が 401・タグ付きメッセージで記録が作られること・`net._http_response` が 200 を確認。逆順（関数を先に deploy）にすると、push までの間トリガーの要求がすべて 401 になりタグ解析・記録・通知が失われる
  - [ ] （任意・オーナー判断）本番の `notification_logs` の Seed.sql 由来の行の確認と削除（確認 SQL・削除文は PR 説明に記載）
