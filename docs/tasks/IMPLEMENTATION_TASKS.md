...
- [x] **5.7 parse-message-tags の本番 URL 直書きと未認証の是正**（2026-09-22 発見・実装、ブランチ `fix/parse-message-tags-webhook`）✅ 完了（#89 マージ・リモート適用・deploy 2026-09-22）
  ...
  - [ ] 残存リスク（本タスクの範囲外）: 受信者 UPDATE ポリシー（`Receivers can mark messages as read`）が列を制限しないため、メッセージの受信者が sender_id / sender_type / content を書き換えて on_message_update → 記録作成を起こせる。`fix/rls-column-write-guards`（列単位の書き込みガード）で塞ぐ前提。それまでは「DB の sender_id を使えば送信者本人の記録しか作られない」は INSERT 経路についてのみ成り立つ
