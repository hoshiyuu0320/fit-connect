...
> **Status:** ✅ 完了（2026-05-04 / PR #47 で `develop/1.0.0` へマージ予定）。本ドキュメントは履歴として保持。
...
- [x] **Step 1: Create migration file with this exact SQL**
...
- [x] **Step 4: Verify existing rows unaffected**
...
- [x] **Step 3: Verify default value applied**
...
- [x] **Step 4: Confirm RLS read access (manual sanity)**
...
- [x] **Step 1: Create initial skeleton**
...
- [x] **Step 2: Serve locally and smoke-test the auth gate**
...
- [x] **Step 1: Insert rate limit check before Claude call (replace the `// TODO` line in B1 step 1's skeleton)**
...
- [x] **Step 1: Add Claude call function above the `Deno.serve` block**
...
- [x] **Step 2: Replace the `// TODO` placeholder in `Deno.serve` with the full call + log + return logic**
...
- [x] **Step 3: Set ANTHROPIC_API_KEY secret locally**
...
- [x] **Step 4: Smoke test with a real call**
...
- [x] **Step 1: Set production secret**
...
- [x] **Step 3: Smoke test against production endpoint**

Same curl as B3 step 4 but pointing at the production URL. Expected 200.

- [x] **Step 4: Verify log row**
...
- [x] **Step 1: Locate the `createMealRecord` function (around line 150) and replace it with**
...
- [x] **Step 2: In the main `Deno.serve` handler, augment `commonData` with metadata before calling `createMealRecord`**
...
- [x] **Step 4: Negative test — message without metadata creates legacy record**
...
- [x] **Step 2: Verify webhook is wired** (sanity)
...
- [x] **Step 1: Add fields to `MealRecord` class**
...
- [x] **Step 3: Run analyzer to confirm no breakage in callers**
...
- [x] **Step 4: Generate code, run test**
...
- [x] **Step 2: Run test (will fail — file missing)**
...
- [x] **Step 4: Generate, test, analyze**
...
- [x] **Step 1: Add an enum and state field at top of `_MealTagFormState`**
...
- [x] **Step 2: Convert `MealTagForm` from `StatefulWidget` to `ConsumerStatefulWidget`**
...
- [x] **Step 3: Update `MealTagForm` constructor to accept an additional callback for "send with PFC"**
...
- [x] **Step 4: Replace `_handleInsert` with branching logic**
...
- [x] **Step 5: Update `build()` to switch on `_phase`**
...
- [x] **Step 6: Implement `_buildLoadingState`**
...
- [x] **Step 1: Create the confirm view widget**
...
- [x] **Step 2: Wire `_buildConfirmState` in `MealTagForm`**
...
- [x] **Step 4: Generate previews exist for State 2/3**
...
- [x] **Step 1: Open the file and read the full `sendMessage` signature**
...
- [x] **Step 2: Identify all existing call sites of `sendMessage`**
...
- [x] **Step 1: Add `Map<String, dynamic>? metadata` parameter to the message INSERT method**
...
- [x] **Step 2: Extend `ChatInput.onSend` signature**
...
- [x] **Step 3: Pass `MealEstimationResult` → metadata via `MealTagForm.onSendWithEstimation`**
...
- [x] **Step 4: Update parent screen (`message_screen.dart`) to forward metadata to repository**
...
- [x] **Step 1: Set test trainer to 'pro'**
...
- [x] **Step 2: Build and run mobile app, log in as that trainer's client**

```bash
cd fit-connect-mobile && flutter run -d <iOS-simulator-id>
```

- [x] **Step 3: Open メッセージ → 食事を記録 → 昼食を選択 → 「牛丼大盛り」と入力 → 挿入**

Expected:
- Sheet shifts to "AI推定中…" with spinner
- Within ~5s, sheet shifts to confirm view with foods list and editable totals

- [x] **Step 4: Edit one of the totals (e.g., kcal: 900 → 850), tap 送信**
...
- [x] **Step 5: Verify ai_estimation_logs has a success row**
...
- [x] **Step 1: Reset trainer to 'free'**

```bash
supabase db execute "UPDATE trainers SET subscription_plan='free' WHERE id='<test-trainer-id>'"
```

- [x] **Step 2: Cold-restart mobile app (so provider re-fetches)**

- [x] **Step 3: Repeat the flow: 食事を記録 → 昼食 → 「サラダ」 → 挿入**
...
- [x] **Step 1: Temporarily break the API key**
...
- [x] **Step 2: Run the AI flow from the app**
...
- [x] **Step 1: Insert 50 dummy logs in last 24h**
...
- [x] **Step 2: Trigger one more AI estimation from the app**

Expected:
- Snackbar: 「AI推定の上限に達しました。しばらくしてからお試しください」
- Sheet returns to input state

- [x] **Step 3: Cleanup test rows**
...
- [x] **Step 1: Mark `2.1` and `2.2` complete**

Replace:
```markdown
- [x] **2.1 Claude API 連携基盤（Supabase Edge Function）**
```
with:
```markdown
...
- [x] **Step 1: After implementation, add a section like**
...
- [x] All Phase A migrations applied locally and remotely
- [x] `estimate-meal-nutrition` deployed to remote Supabase
- [x] `parse-message-tags` deployed with metadata support
- [x] All Flutter unit tests pass: `flutter test`
- [x] `flutter analyze` clean
- [x] H1 (pro plan happy path), H2 (free plan unchanged), H3 (error fallback), H4 (rate limit) all manually verified
- [x] `docs/tasks/IMPLEMENTATION_TASKS.md` updated
- [x] `docs/tasks/lessons.md` updated
- [x] PR opened against `develop/1.0.0` from `feature/ai-meal-estimation-stage1`
