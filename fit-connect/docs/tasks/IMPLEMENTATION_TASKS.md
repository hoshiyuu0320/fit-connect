...
**バージョン**: 2.5
**進捗状況**: 全体 100% 完了
**最終更新**: 2026年2月28日 - スケジュール・ワークアウトプラン画面統合
...
### 2026年2月28日（スケジュール・ワークアウトプラン画面統合）

- スケジュール画面（`/schedule`）とワークアウトプラン画面（`/workoutplan`）を1画面に統合
  - **`AssignmentMiniCard.tsx` 新規作成**: カレンダー上に表示するワークアウトアサインメント（宿題）カード。月/週/日ビュー対応、D&D対応（useDraggable）、色分け（宿題=teal系、セッション=blue系）、ステータスバッジ表示
  - **`CalendarView.tsx` 大幅改修**:
    - クライアント選択フィルタ追加（「全員」がデフォルト、選択すると特定クライアントのみ表示）
    - ワークアウトサイドパネル追加（「ワークアウト」ボタン押下で右からスライドイン、TemplatePanel組み込み）
    - アサインメント取得・表示（月ビュー: 日付セル内、週/日ビュー: 終日エリア）
    - DndContext統合（セッションD&D + テンプレートD&D + アサインメント移動を1つのDndContextで管理）
    - TicketSelectModal組み込み（セッション種別テンプレートD&D時にチケット選択モーダル表示）
  - **`ClientSelector.tsx` 改修**: `showAllOption` prop追加（「全員」選択肢を先頭に表示）
  - **`workoutplan/page.tsx` リダイレクト化**: `/workoutplan` → `/schedule` へリダイレクト
  - **`layout.tsx` メニュー更新**: サイドバーから「ワークアウト」メニュー項目を削除
